<template>
  <div class="moda-feed p-4">
    <h1 class="text-3xl font-bold mb-6">Moda</h1>
    <form @submit.prevent="submitmoda" class="mb-6">
      <div class="mb-4">
        ...
        <input
          ...
          class="border p-2 rounded w-full border-white focus:border-purple-500"
          placeholder="Enter the moda title"
        />
      </div>
      <div class="mb-4">
        ...
        <textarea
          ...
          class="border p-2 rounded w-full border-white focus:border-purple-500"
          placeholder="Enter the moda description"
        ></textarea>
      </div>
  ...
  methods: {
    ...
    submitmoda() {
      if (
        ...
        if (this.newPost.id) {
          this.updatePost({ type: "moda", updatedPost: this.newPost });
        } else {
          const newPost = { ...this.newPost, id: Date.now() };
          this.addPost({ type: "moda", post: newPost });
        }
        this.newPost = {
          id: null,
          title: "",
    ...
    deletePost(postId) {
      this.$store.dispatch("deletePost", { type: "moda", postId });
